...
const Index = () => {
  ...
  const { role, isAdmin, isOperator } = useAuth();

  // Customers skip Decorate (step 5) and Message (step 6) — they only view/download
  const canDecorate = isAdmin || isOperator;
  const canCustomizeMessage = isAdmin || isOperator;

  const handleNextFromPreview = () => {
    if (!canDecorate) {
      // Skip decorate and message, go straight to download
      pb.setStep(7);
    } else {
      pb.nextStep();
    }
  ...
  const handleNextFromDecorate = () => {
    if (!canCustomizeMessage) {
      pb.setStep(7);
    } else {
      pb.nextStep();
    }
